(function() {
  ...
  module.exports = MySQLTable = (function(_super) {
    ...
    */
    ...
    MySQLTable.prototype.mdel = function(ids, cb, opt) {
      var id, options, sql, stmts, _i, _len;
      if (opt == null) {
        opt = {};
      }
      cb = this._wrapCallback(cb);
      ...
      for (_i = 0, _len = ids.length; _i < _len; _i++) {
        id = ids[_i];
        sql.filter(this.sIdField, id).or();
      }
      stmts = [sql.select(false), sql.del()];
      this.factory.exec(stmts, this._handleSingle("mdel", ids, opt, cb));
    };
    ...
    MySQLTable.prototype._afterSave = function(results, id, data, options, cb) {
      var _field, _new, _old, _ref, _ref1, _saveMeta, _val;
      if (id != null) {
      ...
      for (_field in _ref) {
        ...
        if ((id != null) && (_val.fireEventOnChange != null) && _old[_field] !== _new[_field]) {
          this.emit("" + _field + "." + _val.fireEventOnChange, _field, _old[_field], _new[_field], id);
        }
      }
