    ...
    return SQLBuilder = (function(_super) {
      ...
      SQLBuilder.prototype.filter = function(key, pred) {
        ...
        if (_.isObject(key)) {
          ...
          if (this._c.attrNames.length && __indexOf.call(this._c.attrNames, key) < 0) {
            this.warning("invalid filter", this.table, key);
            return this;
          }
          _filter = "" + key + " ";
          if (pred == null) {
      ...
      */
      ...
      SQLBuilder.prototype._validateAttributes = function(isCreate, attrs) {
        ...
        if (_omited.length) {
          this.warning("validateAttributes", "You tried to save a attribute not defined in the model config of `" + this.table + "`", _omited);
        } else {
      ...
      */
      ...
      SQLBuilder.prototype._getSaveVariables = function(attributes) {
        var _cnf, _count, _key, _keys, _m, _operand, _ref, _setval, _val, _vals;
        _keys = [];
        _vals = [];
        for (_key in attributes) {
          ...
          if (_cnf) {
            switch (_cnf.type) {
              case "string":
              case "S":
                if ((_val != null ? _val.slice(0, 3) : void 0) === "IF(") {
                  _vals.push(_val);
                } else {
              ...
              case "number":
              case "N":
                if (_.isString(_val) && (_val != null ? _val.slice(0, 3) : void 0) === "IF(") {
                  _vals.push(_val);
                } else if (_val === "now") {
