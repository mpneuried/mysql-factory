(function() {
  ...
  module.exports = function(options, escape) {
    ...
    return SQLBuilder = (function(superClass) {
      extend(SQLBuilder, superClass);
      ...
      function SQLBuilder(_c) {
        ...
        this._generateSetCommand = bind(this._generateSetCommand, this);
        this._getSaveVariables = bind(this._getSaveVariables, this);
        this._getAttrConfig = bind(this._getAttrConfig, this);
        this._validateAttributes = bind(this._validateAttributes, this);
        this.getJoins = bind(this.getJoins, this);
        this.hasJoins = bind(this.hasJoins, this);
        this.getFilters = bind(this.getFilters, this);
        this.hasFilter = bind(this.hasFilter, this);
        this.getDefaultLimit = bind(this.getDefaultLimit, this);
        this.setDefaultLimit = bind(this.setDefaultLimit, this);
        this.getLimit = bind(this.getLimit, this);
        this.setLimit = bind(this.setLimit, this);
        this.getUseFieldsets = bind(this.getUseFieldsets, this);
        this.setUseFieldsets = bind(this.setUseFieldsets, this);
        ...
        this.getOrderBy = bind(this.getOrderBy, this);
        this.getForward = bind(this.getForward, this);
        this.setForward = bind(this.setForward, this);
        this.getOrderField = bind(this.getOrderField, this);
        this.setOrderField = bind(this.setOrderField, this);
        this.getArrayAttrKeys = bind(this.getArrayAttrKeys, this);
        this.getAttrKeys = bind(this.getAttrKeys, this);
        this.getAttrs = bind(this.getAttrs, this);
        this.setAttrs = bind(this.setAttrs, this);
        this.getIdField = bind(this.getIdField, this);
        this.setIdField = bind(this.setIdField, this);
        this.getTable = bind(this.getTable, this);
        this.setTable = bind(this.setTable, this);
        this.hasField = bind(this.hasField, this);
        this.convertToType = bind(this.convertToType, this);
        this.setToArray = bind(this.setToArray, this);
        this.filterGroup = bind(this.filterGroup, this);
        this.and = bind(this.and, this);
        this.or = bind(this.or, this);
        this.addJoin = bind(this.addJoin, this);
        ...
        (base = this._c).attrs || (base.attrs = []);
        (base1 = this._c).attrKeys || (base1.attrKeys = []);
        (base2 = this._c).attrNames || (base2.attrNames = []);
        (base3 = this._c).attrsArrayKeys || (base3.attrsArrayKeys = []);
        (base4 = this._c).fieldsets || (base4.fieldsets = {});
        (base5 = this._c).fieldlist || (base5.fieldlist = []);
        (base6 = this._c).joins || (base6.joins = []);
      ...
      SQLBuilder.prototype.insert = function(attributes) {
        var _keys, _vals, ref, statement;
        attributes = this._validateAttributes(true, attributes);
        statement = [];
        statement.push("INSERT INTO " + this.table);
        ref = this._getSaveVariables(attributes, true), _keys = ref[0], _vals = ref[1];
      ...
      SQLBuilder.prototype.update = function(attributes) {
        var _idx, _key, _keys, _sets, _vals, i, len, ref, statement;
        attributes = this._validateAttributes(false, attributes);
        statement = [];
        statement.push("UPDATE " + this.table);
        ref = this._getSaveVariables(attributes, false), _keys = ref[0], _vals = ref[1];
        _sets = [];
        for (_idx = i = 0, len = _keys.length; i < len; _idx = ++i) {
          _key = _keys[_idx];
      ...
      SQLBuilder.prototype.filter = function(key, pred) {
        var _cbn, _filter, _k, _operand, _pred, _val, base, ref, subtable;
        (base = this._c).filters || (base.filters = []);
        ...
        if (_.isObject(key)) {
          ...
          if (this._c.attrNames.length && indexOf.call(this._c.attrNames, key) < 0) {
          ...
          if ((ref = this._c.filters) != null ? ref.length : void 0) {
      ...
      SQLBuilder.prototype.filterGroup = function(newGroup) {
        ...
        if (newGroup) {
          this._c._filterGroup = (((ref = this._c.filters) != null ? ref.length : void 0) || 0) + _add;
        }
        return this;
      };
      ...
      SQLBuilder.prototype.convertToType = function(key, value) {
        var _cnf, _item, _key, _ret, _val, error, i, len;
        if (_.isArray(key)) {
          _ret = [];
          ...
          for (_key in key) {
            _val = key[_key];
            if (indexOf.call(this.attrKeys, _key) >= 0) {
            ...
            switch (_cnf.type) {
              ...
              case "D":
                ...
                if (_.isDate(value)) {
                  ...
                  return moment(value, this.config.dateFormats).toDate();
                }
                break;
              case "array":
              case "A":
      ...
      SQLBuilder.prototype.hasField = function(field) {
        return indexOf.call(this.attrKeys, field) >= 0;
      };
      ...
      SQLBuilder.prototype.setAttrs = function(_attrs) {
        var _attr, _defAttrCnf, attr, fldst, i, key, len, ref, ref1;
      ...
      SQLBuilder.prototype.getOrderBy = function() {
        var _afields, _dir, _ofield, i, len, ofield, ref;
        _dir = "DESC";
        if (this.forward) {
          _dir = "ASC";
        }
        _ofield = this.orderfield;
        _afields = [];
        if (_ofield.indexOf(",") >= 0) {
          ref = _ofield.split(",");
          for (i = 0, len = ref.length; i < len; i++) {
            ofield = ref[i];
            _afields.push(this.table + "." + (utils.trim(ofield)) + " " + _dir);
          }
          return "ORDER BY " + (_afields.join(", "));
        } else {
          return "ORDER BY " + this.table + "." + this.orderfield + " " + _dir;
        }
      };
      ...
      SQLBuilder.prototype.getFields = function() {
        var _fdls, field, i, len, ref, ref1;
        if ((ref = this._c.fields) != null ? ref.length : void 0) {
          _fdls = [];
          ref1 = this._c.fields;
          for (i = 0, len = ref1.length; i < len; i++) {
            field = ref1[i];
      ...
      SQLBuilder.prototype._getSaveVariables = function(attributes, isInsert) {
        var _cnf, _count, _d, _key, _keys, _m, _operand, _setval, _val, _vals, ref;
        if (isInsert == null) {
          isInsert = false;
        }
        _keys = [];
            ...
            switch (_cnf.type) {
              ...
              case "T":
                if (_val === "now") {
                  ...
                  _d = moment(_val, _cnf.dateFormat || this.config.dateFormats);
              ...
              case "D":
                ...
                if (_.isDate(_val)) {
                  _vals.push(escape(_val));
                  _keys.push(_key);
                } else if (_.isDate((ref = (_m = moment(_val, this.config.dateFormats))) != null ? ref._d : void 0)) {
      ...
      SQLBuilder.prototype._generateSetCommand = function(key, inp, dlm) {
        var _add, _inp, _set, added, i, j, len, len1, ref, ref1, usedRem;
          ...
          if (inp["$reset"]) {
            ...
            if (inp["$rem"] != null) {
              if (_.isArray(inp["$rem"])) {
                ref1 = _.difference(_.uniq(inp["$rem"]), added);
