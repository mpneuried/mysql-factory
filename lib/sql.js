    ...
    return SQLBuilder = (function(_super) {
      ...
      */
      ...
      SQLBuilder.prototype._getSaveVariables = function(attributes) {
        var _cnf, _key, _keys, _m, _ref, _ref1, _ref2, _setval, _val, _vals;
        _keys = [];
        _vals = [];
        for (_key in attributes) {
          ...
          if (_cnf) {
            switch (_cnf.type) {
              ...
              case "S":
              case "N":
                if (((_ref = _cnf.type) === "string" || _ref === "S") && (_val != null ? _val.slice(0, 3) : void 0) === "IF(") {
                  _vals.push(_val);
                } else if (_val === "now") {
                  _vals.push("UNIX_TIMESTAMP()*1000");
                } else if (_val === "incr" && ((_ref1 = _cnf.type) === "number" || _ref1 === "N")) {
                  _vals.push("IF( " + _key + " is NULL, 0, " + _key + " + 1 )");
                } else if (_val === "decr" && ((_ref2 = _cnf.type) === "number" || _ref2 === "N")) {
                  _vals.push("IF( " + _key + " is NULL, 0, " + _key + " - 1 )");
                } else {
