...
// MP: borrowed method from https://github.com/coolaj86/node-bufferjs/blob/master/bufferjs/concat.js
var BuffConcat = function BuffConcat(bufs) {
    ...
    case Parser.ROW_DATA_PACKET:
      ...
      packet.on('data', function(buffer, remaining) {
        if (!field) {
          ...
          // MP: changed form empty string to empty buffer
          row[field.name] = new Buffer('');
        }

        if (buffer) {
          // MP: concart buffers not strings
          row[field.name] = BuffConcat( row[field.name], buffer );
        } else {
          row[field.name] = null;
        }
        
		// MP: if there are no packages left convert data to uft-8
		if( remaining === 0 ){
			if( row[field.name] instanceof Buffer ){
